...
class PrefixedIncludingSize(Subconstruct):
    ...
    def _build(self, obj, stream, context, path):
        try:
            ...
            if obj.type == b'uuid' and type(obj.data) != bytes:
                obj.data = UUIDSubBoxStruct.build1(obj, context)

            self.subcon._build(obj, stream, context, path)

            # if hasattr(obj, 'children') and obj.children is not None and len(obj.children) != 0:
            #     for child in obj.children:
            #         if child.type == b'uuid' and type(child.data) != bytes:
            #             child.data = UUIDSubBoxStruct.build1(child, context)
            #         else:
            #             self.subcon._build(child, stream, context, path)
...
VPCConfigBox = Struct(
    ...
    "level" / Int8ub,
    EmbeddedBitStruct(
        "bitDepth" / Default(BitsInteger(4), 0),
        "chromaSubsampling" / Default(BitsInteger(3), 0),
        "videoFullRangeFlag" / Default(BitsInteger(1), 0),
    ),
    "colourPrimaries" / Int8ub,
    "transferCharacteristics" / Int8ub,
    "matrixCoefficients" / Int8ub,
    "codecIntializationDataSize" / Int16ub,
    "codecIntializationData" / Array(this.codecIntializationDataSize, Byte)
)

VPCCSampleEntryBox = Struct(
    "type" / Const(b"vpcC"),
    "version" / Default(ByteSwapped(Int32ub), 1),
    Embedded(Switch(this.version, {
        0: Struct('Deprecated' / GreedyRange(Byte)),  # Version 0 is deprecated and should not be used., from: https://www.webmproject.org/vp9/mp4/

        1: Struct('VPCConfig' / VPCConfigBox),
    }))
)

AVCConfigBox = Struct(
    "version" / Const(Int8ub, 1),
    "profile" / Default(Int8ub, 77),
    "compatibility" / Default(Int8ub, 64),
    "level" / Default(Int8ub, 31),
    EmbeddedBitStruct(
        "lengthSizeMinusOnePaddingBits" / Default(BitsInteger(6), 63),
        "lengthSizeMinusOne" / Default(BitsInteger(2), 3),
        "numberOfSequenceParameterSetsPaddingBits" / Default(BitsInteger(3), 7),
        "numberOfSeuqenceParameterSets" / Default(BitsInteger(5), 1),
    ),
    "sequenceParameterSets" / Array(this.numberOfSeuqenceParameterSets, PrefixedArray(Int16ub, Byte)),
    "numberOfPictureParameterSets" / Default(Int8ub, 1),
    "pictureParameterSets" / Array(this.numberOfPictureParameterSets, PrefixedArray(Int16ub, Byte)),
    "extraData" / GreedyBytes
)

AVCCSampleEntryBox = Struct(
    "type" / Const(b"avcC"),
...
VisualSampleEntry = Struct(
    "reservered0" / Int16ub,
    "reservered1" / Int16ub,
    "predefined" / Array(3, Int32ub),
    "width" / Int16ub,
    "height" / Int16ub,
    "horizresolution" / Default(Int16ub, 72),  # TODO: actually a fixed point decimal
    Padding(2),
    "vertresolution" / Default(Int16ub, 72),  # TODO: actually a fixed point decimal
    Padding(2),
    "reservered2" / Int32ub,
    "frameCount" / Int16ub,
    "compressornameDisplayAbleData" / Default(String(32, padchar=b" "), ""),
    "depth" / Default(Int16ub, 24),
    "color_table_id" / Default(Int16sb, -1),
    "children" / GreedyRange(LazyBound(lambda x: Box))
)
...
SampleEntryBox = PrefixedIncludingSize(Int32ub, Struct(
    ...
    Embedded(Switch(this.type, {
        ...
        b"avc1": VisualSampleEntry,
        b"encv": VisualSampleEntry,
        b"vp09": VisualSampleEntry,
    # }, Struct("extraData" / GreedyBytes)))
    })),
))

ENCVSampleEntryBox = Struct(
    "type" / Const(b"encv"),
    Padding(6, pattern=b"\x00"),
    "data_reference_index" / Default(Int16ub, 1),
    Embedded(
...
class UUIDSubBoxStruct(Subconstruct):
    __slots__ = ['type']

    def __init__(self, type, subcon):
        super(UUIDSubBoxStruct, self).__init__(subcon)
        self.type = type
    ...
    @staticmethod
    def build1(obj, context):
        data = None
        if obj.data.type == b'tenc':
            data = UUIDTrackEncryptionBox.build(obj.data, context)
        elif obj.data.type == b'senc':
            data = UUIDSampleEncryptionBox.build(obj.data, context)
        elif obj.data.type == b'pssh':
            data = UUIDProtectionSystemHeaderBox.build(obj.data, context)

        return data


ProtectionSystemHeaderBoxData = Struct(
...
ProtectionSystemHeaderBox = Struct(
    ...
    Embedded(ProtectionSystemHeaderBoxData)
)

TrackEncryptionBoxData = Struct(
    "version" / Const(Int8ub, 0),
    "flags" / Const(Int24ub, 0),
...
TrackEncryptionBox = Struct(
    ...
    Embedded(TrackEncryptionBoxData)
)

SampleEncryptionBoxData = Struct(
    "version" / Const(Int8ub, 0),
    "flags" / BitStruct(
        Padding(22),
...
SampleEncryptionBox = Struct(
    ...
    Embedded(SampleEncryptionBoxData)
)

UUIDProtectionSystemHeaderBox = UUIDSubBoxStruct(b"pssh",
                                                 ProtectionSystemHeaderBoxData)
UUIDSampleEncryptionBox = UUIDSubBoxStruct(b"senc",
                                           SampleEncryptionBoxData)
UUIDTrackEncryptionBox = UUIDSubBoxStruct(b"tenc",
                                          TrackEncryptionBoxData)
...
UUIDBox = Struct(
    ...
    "data" / Switch(this.extended_type, {
        UUID("A2394F52-5A9B-4F14-A244-6C427C648DF4"): UUIDSampleEncryptionBox,
        UUID("D08A4F18-10F3-4A82-B6C8-32D8ABA183D3"): UUIDProtectionSystemHeaderBox,
        UUID("8974DBCE-7BE7-4C51-84F9-7148F9882554"): UUIDTrackEncryptionBox
    }, GreedyBytes)
)
...
Box = PrefixedIncludingSize(Int32ub, Struct(
    ...
    Embedded(Switch(this.type, {
        ...
        b"encv": ENCVSampleEntryBox,
        b"avcC": AVCCSampleEntryBox,
        b"avc1": AVC1SampleEntryBox,
        b"vpcC": VPCCSampleEntryBox,
        b"vp09": VP09SampleEntryBox,
    }, default=RawBox)),
    "end" / Tell
))
